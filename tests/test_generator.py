...
def test_invoke_json_qa(monkeypatch, sample_context, sample_qa_response):
    ...
    monkeypatch.setattr(QuestionGenerator, "__init__", mock_init)
    monkeypatch.setattr("llm_output_parser.parse_json", lambda _: json_response if isinstance(json_response, str) else json.dumps(json_response))
...
def test_invoke_json_mcq(monkeypatch, sample_context, sample_mcq_response):
    ...
    generator = QuestionGenerator(question_type=QuestionType.MCQ, output_type=OutputType.JSON)
    result = generator.invoke(sample_context)
    
    assert isinstance(result, dict)
    assert "mcq_questions" in result
    assert len(result["mcq_questions"]) == 1
    assert result["mcq_questions"][0]["question_text"] == "What was the purpose of pyramids in Ancient Egypt?"
